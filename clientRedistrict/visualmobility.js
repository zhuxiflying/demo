$(function(){
	var AppView = Backbone.View.extend({
		...
		updateCanvas: function(){
			...
			$('#canvas .row.halfheight .cell.halfwidth').height(height/2-8).width(Math.floor(width)).css("display", "inline-block");
  			$('#canvas .row.halfheight .cell.fullwidth').height(height/2.1-8).width(Math.floor(width-8)).css("display", "inline-block");
  			$('#canvas .row.fullheight .cell.halfwidth').height(height-4).width(Math.floor(width)).css("display", "inline-block");
		...
		openCanvasSettings: function(index){
			...
			switch(settings.id){
				case "plot":
					...
					var testValues = [ {key: "distance", value: "Distance"},
									{key: "start_date", value: "Date"},
									{key: "hour", value: "Time"},
									{key: "carbon", value: "CO2"},
									{key: "extract(epoch from tracks.time)", value: "Duration"}];			
					...
					*/
					...
					var testSelect = $('div#plots select#test');
					testSelect.html("");
					_.each(testValues, function(option){
						testSelect.append('<option id="' + option.key + '"' + (option.key==settings.color ? ' selected="selected"' : '') + '>' + option.value + '</option>');
					});
	...
	var SideMenuView = Backbone.View.extend({
	    ...
	    initialize: function() {
	    	...
	    	this.tabs = [$("#filterTab"), $("#configureTab"), $("#saveTab"), $("#distanceTab"), $("#boundaryTab")];
	    	this.tabsContent = [$("#filterContent"), $("#configureContent"), $("#saveContent"), $("#distanceContent"),$("#boundaryContent")];
		...
		sidepanelBtnClicked: function() {
			...
			if(this.$el.hasClass("open")){
				...
				$('#canvas .cell.halfwidth').animate({width: Math.floor(width-8)}, 1000, 'swing', finishCallback);
